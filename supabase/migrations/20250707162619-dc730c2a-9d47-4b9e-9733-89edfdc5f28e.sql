...
CREATE TABLE public.hipaa_access_logs (
  ...
  data_accessed TEXT, -- What specific data was accessed
...
-- Create role permissions table for enhanced RBAC
CREATE TABLE public.role_permissions (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  role user_role NOT NULL,
  permission_category TEXT NOT NULL, -- e.g., 'patient_records', 'billing', 'reports'
  permission_action TEXT NOT NULL, -- e.g., 'read', 'write', 'delete', 'export'
  resource_scope TEXT DEFAULT 'all', -- 'all', 'own', 'department'
...
INSERT INTO public.role_permissions (role, permission_category, permission_action, resource_scope) VALUES
...
('Practice Administrator', 'patient_records', 'export', 'all'),
('Practice Administrator', 'billing', 'read', 'all'),
('Practice Administrator', 'billing', 'write', 'all'),
('Practice Administrator', 'reports', 'read', 'all'),
...
('Clinical Administrator', 'patient_records', 'export', 'department'),
('Clinical Administrator', 'reports', 'read', 'department'),
('Clinical Administrator', 'reports', 'generate', 'department'),
...
('Billing Specialist', 'patient_records', 'read', 'billing_only'),
('Billing Specialist', 'reports', 'read', 'billing'),

('Supervisor', 'patient_records', 'read', 'supervised'),
('Supervisor', 'patient_records', 'write', 'supervised'),
('Supervisor', 'reports', 'read', 'supervised');
...
('clients', 'first_name', 'phi', true, 2555), -- 7 years
...
-- Create RLS policies for security audit logs (admin only)
...
-- Create RLS policies for role permissions (admin only)
...
-- Create RLS policies for data classifications (admin only)
...
BEGIN
  ...
  INSERT INTO public.hipaa_access_logs (
    ...
    p_data_accessed,
    p_purpose,
    '127.0.0.1'::inet -- This would be actual IP in production
  );
END;
...
DECLARE
  current_user_id UUID;
BEGIN
  -- Get current user ID (optional for system events)
  SELECT id INTO current_user_id 
  FROM public.users 
  WHERE auth_user_id = auth.uid();
