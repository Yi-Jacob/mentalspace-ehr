...
export class SessionCompletionController {
  ...
  @Get()
  async getAllSessionCompletions(@Query('status') status?: string, @Query('providerId') providerId?: string) {
    return this.sessionCompletionService.getAllSessionCompletions(status, providerId);
  }
