...
// Generic hook for note forms
export const useUnifiedNoteForm = <T extends BaseFormData>(
  ...
  // Section progress calculation (can be overridden by specific note types)
  const getSectionProgress = () => {
    return sections.map((section, index) => {
      // Basic completion logic - can be overridden
      let isComplete = false;
      let hasRequiredFields = true;
      let requiredFieldsComplete = true;

      // This is a basic implementation - specific note types should override this
      switch (section.id) {
        case 'finalize':
